...
/* ---------- Ambient typings for your CustomEvents (silences TS) ---------- */
declare global {
...
// ---- Touch detection (used throughout) ----
const IS_TOUCH =
...
const BOARD_ALT = 2.8;  // grounded boards' center height (used by GroundedWhiteboards)
const TITLE_ALT = 10;
...
const MAX_STEP = 0.45;         // allow tiny step-down without falling
const MAX_UP_SNAP = 0.5;       // never snap UP >50cm unless ladder grace
const ROOF_GRACE_MS = 1200;    // grace after ladder toggle
...
function TC_LeftStick() {
  ...
  const onMove = (e: TouchEvent) => {
    ...
    const radius = 60;
    let x = dx / radius, y = -dy / radius; // invert Y so up = forward
  ...
  useEffect(() => {
    ...
    add("touchstart", onStart); add("touchmove", onMove); add("touchend", onEnd); add("touchcancel", onEnd);
    return () => { rm("touchstart", onStart); rm("touchmove", onMove); rm("touchend", onEnd); rm("touchcancel", onEnd); };
  }, []);
...
function TC_ActionBar() {
  ...
  const pulse = (key: string, holdMs = 0) => {
    tc_keyDown(key);
    if (holdMs) setTimeout(() => tc_keyUp(key), holdMs);
  };
  return (
    <div style={tc_bar}>
      <button style={tc_btn} onTouchStart={(e)=>{e.preventDefault(); pulse(" ");}}>Jump</button>
      <button style={{...tc_btn, opacity: sprinting?1:0.85}}
              onTouchStart={(e)=>{e.preventDefault(); setSprinting(true); tc_keyDown("Shift");}}
              onTouchEnd={(e)=>{e.preventDefault(); setSprinting(false); tc_keyUp("Shift");}}>
        Sprint
      </button>
      <button style={tc_btn} onTouchStart={(e)=>{e.preventDefault(); pulse("e");}}>Interact</button>
      <button style={tc_btn} onTouchStart={(e)=>{e.preventDefault(); pulse("q");}}>Enter/Exit</button>
      <button style={tc_btn} onTouchStart={(e)=>{e.preventDefault(); pulse("f");}}>Climb</button>
    </div>
  );
}
...
const tc_leftPad: React.CSSProperties = {
  position:"absolute", left:12, bottom:12, width:140, height:140, borderRadius:90,
  background:"rgba(15,23,42,0.25)", border:"2px solid rgba(148,163,184,0.5)",
  pointerEvents:"auto", touchAction:"none",
};
const tc_rightPad: React.CSSProperties = {
  position:"absolute", right:12, bottom:12, width:160, height:160, borderRadius:12,
  background:"rgba(15,23,42,0.18)", border:"2px solid rgba(148,163,184,0.4)",
  pointerEvents:"auto", touchAction:"none",
};
const tc_bar: React.CSSProperties = {
  position:"absolute", right:12, top:12, display:"flex", gap:8, flexWrap:"wrap", pointerEvents:"auto",
};
const tc_btn: React.CSSProperties = {
  fontFamily:"monospace", fontWeight:900, padding:"10px 12px",
  background:"rgba(34,197,94,0.9)", border:"3px solid #14532d", color:"#0b2e13", borderRadius:10, touchAction:"none",
};
const tc_ring: React.CSSProperties = {
  position:"absolute", left:"50%", top:"50%", width:110, height:110, marginLeft:-55, marginTop:-55,
  border:"2px dashed rgba(226,232,240,0.5)", borderRadius:80,
};

/* ===================== App (with mobile support) ===================== */
export default function App() {
  ...
  const topBtnPos = useMemo(()=>getTopButtonPos(),[]);           // now actually used below
  ...
  ? "linear-gradient(#0b1220, #1a237e)"
  : "linear-gradient(#87ceeb, #1e90ff)";
  ...
  if (!started) {
    return (
      <>
        <TitleScreen
          onContinue={() => {
            setLowSpec(IS_TOUCH ? true : false); // auto-lite on mobile
            setStarted(true);
          }}
          onLiteMode={() => {
            setLowSpec(true);
    ...
    <div style={{ width: "100vw", height: "100vh", overflow: "hidden" }}>
      ...
      {!activeBoard && !IS_TOUCH && (
        <div style={{ position: "fixed", top: 10, left: "50%", transform: "translateX(-50%)", background: "rgba(0,0,0,0.5)", color: "#fff", padding: "8px 12px", borderRadius: 10, zIndex: 10, fontSize: 14 }}>
          Click to lock the mouse · WASD move · Space jump · F to toggle ladder climb · Press <b>E</b> near the golden button · Q to enter/exit · Shift sprint · M mute
        </div>
      )}
      {!activeBoard && IS_TOUCH && (
        <div style={{ position: "fixed", top: 10, left: "50%", transform: "translateX(-50%)", background: "rgba(0,0,0,0.45)", color: "#fff", padding: "6px 10px", borderRadius: 10, zIndex: 10, fontSize: 12 }}>
          Left pad: move · Right pad: look · Buttons: Jump / Sprint / E / Q / F
        </div>
      )}
      ...
      >
        ...
        {/* Use the memoized topBtnPos so it's not "declared but never read" */}
        <InteractAtPoint
          target={topBtnPos}
          enabled={!activeBoard}
          keyName="e"
      ...
      {/* Mobile overlay (drawn above Canvas) */}
...
function InteractAtPoint({
  target, enabled, keyName = "e", range = 2.0, label, onTrigger, setPrompt,
}: {
  ...
  useFrame(() => {
    if (!enabled) { if (inRange.current) { inRange.current = false; setPrompt(null); } return; }
...
function ImageModal({ img, caption, onClose, darkMode }: { img: string; caption: string; onClose: ()=>void; darkMode:boolean }) {
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => { if (e.key === "q" || e.key === "Escape") onClose(); };
    window.addEventListener("keydown", onKey);
    return () => window.removeEventListener("keydown", onKey);
  }, [onClose]);
...
function WhiteboardModal({ config, onClose, darkMode }: { config: (typeof WHITEBOARD_CONFIG)[0]; onClose: () => void; darkMode: boolean; }) {
  ...
  const paper = darkMode ? "#0f172a" : "#d6c2a5", ink = darkMode ? "#e5e7eb" : "#111827", frame = darkMode ? "#0b1220" : "#0f172a", panel = darkMode ? "#131a2a" : "#fefefe";
  ...
  return (
    <div onMouseDown={(e) => { if (e.target === e.currentTarget) onClose(); }}
      ...
      <div onMouseDown={(e) => e.stopPropagation()}
           style={{ position:"relative", width:"92vw", height:"92vh", background: paper, borderRadius:0, overflow:"hidden", display:"flex", flexDirection:"column", ...pixelBorder(6), ...pixelTile }}>
        <div style={grassStrip} />
        <button onClick={onClose} title="ESC also closes"
          style={{ position:"absolute", top:16, right:16, padding:"10px 18px", background:"#22c55e", color:"#0b2e13", border:"4px solid #14532d", cursor:"pointer", fontFamily:"monospace", fontWeight:900, letterSpacing:1, textTransform:"uppercase", imageRendering:"pixelated", ...pixelBorder(2) }}>
          EXIT
        </button>
        ...
        <div style={{ display:"flex", gap:"1.5rem", padding:"1rem", flex:1, overflow:"hidden" }}>
          ...
          <div style={{ flex:1, display:"flex", flexDirection:"column", minWidth:0 }}>
            <div style={{ background: panel, padding:"0.75rem 1rem", border:`4px solid ${frame}`, fontFamily:"monospace", fontWeight:900, fontSize:"1.8rem", letterSpacing:1, color: darkMode ? "#ffffff" : "#0f172a", ...pixelBorder(2) }}>
              {config.title.toUpperCase()}
            </div>

            <div style={{ marginTop:"1rem", background:"#ffffff", padding:"1rem", border:`4px solid ${frame}`, fontFamily:"monospace", color:ink, lineHeight:1.7, flex:1, overflow:"auto", ...pixelBorder(2) }}>
              {Array.isArray((config as any).sections) ? (
                (config as any).sections.map((sec: any, i: number) => (
                  <div key={i} style={{ marginBottom:"1.1rem" }}>
                    <div style={{ fontSize:"1.35rem", fontWeight:900, marginBottom:6, color:ink }}>
                      {sec.url ? (
                        <a href={sec.url} target="_blank" rel="noopener noreferrer" style={{ color: darkMode ? "#93c5fd" : "#0f172a", textDecoration: "underline" }}>{sec.title}</a>
                      ) : sec.title}
                    </div>
                ...
                <div style={{ fontSize:"1.05rem", color:ink }}>No content yet. Add <code>sections</code> to this board to populate it.</div>
              )}
              <div style={{ height: 24 }} />
          ...
          <div style={{ width:420, display:"flex", flexDirection:"column", alignItems:"stretch", gap:12, minWidth:0, overflow:"auto" }}>
            {(((config as any).images) ?? ((config as any).image ? [(config as any).image] : [])).map((src: string, idx: number) => (
              <div key={idx} style={{ width:"100%", border:`4px solid ${frame}`, background:"#ffffff", boxShadow: "0 0 0 6px #111827, 0 0 0 12px #6b7280, 0 0 0 18px #111827" }}>
...
/* ---------- Pointer-lock mouse look + mobile look ---------- */
function MouseLookControls({ enabled, initialYaw = 0, initialPitch = -0.1 }: { enabled: boolean; initialYaw?: number; initialPitch?: number; }) {
  ...
  useEffect(() => {
    ...
    window.addEventListener("relock-pointer", onRelockEvent as any);
    return () => {
      canvas.removeEventListener("click", onClick);
      document.removeEventListener("pointerlockchange", onLockChange);
      window.removeEventListener("mousemove", onMouseMove);
      window.removeEventListener("relock-pointer", onRelockEvent as any);
    };
  }, [enabled, gl]);
...
/* ---------- Movement (WASD + JUMP + Ladder toggle F + Sprint + mobile joystick) ---------- */
function MovementControls({ enabled, speed = 3.5, sprint = 1.9, insideHouseId }: { enabled: boolean; speed?: number; sprint?: number; insideHouseId?: string | null}) {
  ...
  const baseEye = 1.6, gravity = 20, jumpSpeed = 7.5, climbSpeed = 3.0, radius = 0.4;
  const climbing = useRef(false);
  const climbVolRef = useRef<AABB | null>(null);

  const joy = useRef({ x: 0, y: 0 });
  useEffect(() => {
    const onJoy = (e: any) => { joy.current = e.detail || { x:0, y:0 }; };
  ...
  const isDown = (name: string) => !!(keys.current[name] || keys.current[name.toLowerCase()]);

  function collidesXYAt(x: number, z: number) {
    const yMin = (camera.position.y - baseEye) + 0.02;
    const yMax = camera.position.y - 0.02;
  ...
  function groundAtLimited(x: number, z: number, footY: number, allowHighSnap: boolean) {
    ...
    for (const a of walkList) {
      const minX = a.min[0] - EDGE_PAD - probe, maxX = a.max[0] + EDGE_PAD + probe;
      const minZ = a.min[2] - EDGE_PAD - probe, maxZ = a.max[2] + EDGE_PAD + probe;
  ...
  useFrame(() => {
    const now = performance.now(), dt = (now - last.current) / 1000; last.current = now;
    if (!enabled) return;
    ...
    if (!collidesXYAt(next.x, next.z)) { nx = next.x; nz = next.z; }
    else { if (!collidesXYAt(next.x, camera.position.z)) nx = next.x; if (!collidesXYAt(camera.position.x, next.z)) nz = next.z; }
    ...
    if (climbing.current && hereClimb) {
      ...
      const minY = hereClimb.min[1] + baseEye, maxY = hereClimb.max[1] + baseEye + 0.2;
      ...
      if (vY.current <= 0) {
        ...
        if (dist <= GROUND_SNAP) { camera.position.y = minY; vY.current = 0; }
        else if (dist > 0 && dist < MAX_STEP) { camera.position.y = minY; vY.current = 0; }
      }

      const onGround = Math.abs(camera.position.y - minY) < 0.005 || camera.position.y < minY;
      if (onGround) { camera.position.y = minY; vY.current = 0; }
...
function World({ darkMode,enabled,setPrompt, onDefs,lowSpec = false,insideHouseId,}: { darkMode: boolean;enabled:boolean;setPrompt: (s: string | null) => void; onDefs: (defs: HouseDef[]) => void;lowSpec?: boolean; insideHouseId: string | null;}) {
  ...
  const houseDefs = useMemo<FullHouseDef[]>(() => {
    const raw = [{ id: "house-0", x: -16, z: -12 }, { id: "house-1", x:  16, z: -10 }, { id: "house-2", x: -14, z:  14 }, { id: "house-3", x:  14, z:  14 }];
    ...
    return raw.map(h => {
      ...
      const ladderX = h.x + baseW*0.35, ladderZ = h.z + baseD/2 + ld/2 + 0.02;
      return { ...h, baseW, baseH, baseD, roofT, ld, ladderX, ladderZ, doorWorld, insideSpawn, interiorLight };
    });
  }, []);
  useEffect(() => {
    onDefs(houseDefs.map(h => ({ id:h.id, x:h.x, z:h.z, doorWorld:h.doorWorld, insideSpawn:h.insideSpawn, interiorLight:h.interiorLight })));
  }, [onDefs, houseDefs]);

  useEffect(() => () => { groundTex.dispose?.(); }, [groundTex]);
  useEffect(() => {
    const blockers: AABB[] = [], walk: AABB[] = [], climb: AABB[] = [];
    ...
    [...fixedTrees, ...ringTrees].forEach(([x,z])=>{
      const w=0.6, d=0.6, h=2.0; blockers.push({ min:[x-w/2, 0, z-d/2], max:[x+w/2, h, z+d/2] });
    });
    ...
    houseDefs.forEach(h => {
      ...
      walk.push({ min:[x-(baseW+over)/2 + inset, baseH, z-(baseD+over)/2 + inset], max:[x+(baseW+over)/2 - inset, baseH + 0.12, z+(baseD+over)/2 - inset] });
    ...
    getParkourDefs().forEach(b=>{
      ...
      const overhang = 0.06;
      walk.push({ min: [b.x - b.w/2 - overhang, b.h - 0.02, b.z - b.d/2 - overhang], max: [b.x + b.w/2 + overhang, b.h + 0.20, b.z + b.d/2 + overhang] });
    });
  ...
  return (
    <group>
      ...
      {darkMode && houseDefs.map(h => (
        <pointLight key={h.id} position={(h as any).interiorLight} intensity={0.9} distance={10} color={"#ffd27a"} />
      ))}
    </group>
  );
}
...
function Trees({ darkMode, houseDefs }: { darkMode: boolean; houseDefs: { id: string; x: number; z: number }[]; }) {
  ...
  [...fixed, ...ring].forEach(([x, z], i) => {
    if (isInsideAnyHouse(x, z)) return;
...
function Tree({ position = [0,0,0] as [number,number,number], darkMode }: { position:[number,number,number], darkMode:boolean }) {
  const trunk = "#8b5a2b";
  const leaf1 = darkMode ? "#013220" : "#2fad4e";
...
function House({ position = [0,0,0] as [number,number,number], darkMode, insideActive = false, }: { position: [number,number,number]; darkMode: boolean; insideActive?: boolean; }) {
  ...
  const baseW=8, baseH=4.4, baseD=8, ridgeY = baseH, roofT = 0.36;
  return (
    <group position={position}>
      ...
      <group position={[baseW*0.35, 1.6, baseD/2 + 0.02]}>
        ...
        {Array.from({length:7}).map((_,i)=> (
          <mesh key={i} position={[0, -1.4 + i*(3.0/6), 0]}><boxGeometry args={[0.7, 0.08, 0.06]} /><meshBasicMaterial map={plank} /></mesh>
        ))}
      </group>
    </group>
  );
...
function ParkourBoxes(){
  ...
  useFrame(({ clock }) => { if (!ringRef.current) return; const s = 1 + Math.sin(clock.getElapsedTime() * 3) * 0.08; ringRef.current.scale.set(s, 1, s); });
  return (
    <group>
      {defs.map((b,i)=> (
        <mesh key={i} position={[b.x, b.h/2, b.z]}><boxGeometry args={[b.w, b.h, b.d]} /><meshBasicMaterial map={plank} /></mesh>
      ))}
      {top && (
        <group position={[top.x, top.h + 0.25, top.z]}>
          <mesh><cylinderGeometry args={[0.45,0.45,0.2,20]} /><meshBasicMaterial color="#f59e0b" /></mesh>
          <mesh ref={ringRef} position={[0,0.02,0]}><torusGeometry args={[0.65, 0.05, 10, 40]} /><meshBasicMaterial color="#fde68a" /></mesh>
        </group>
      )}
    </group>
  );
...
function Cloud({ position=[0,0,0] as [number,number,number], darkMode }: { position:[number,number,number]; darkMode:boolean }){
  const cloud = darkMode ? "#d1d5db" : "#ffffff";
  return (
    <group position={position}>
      {[[0,0,0],[1.2,0.3,0.4],[-1,0.2,-0.4],[0.2,-0.1,0.9]].map((o,i)=>(
        <mesh key={i} position={[o[0],o[1],o[2]]}><boxGeometry args={[2,1,1]} /><meshBasicMaterial color={cloud} /></mesh>
      ))}
    </group>
  );
}
...
function CloudField({ darkMode }: { darkMode: boolean }){
  ...
  grid.forEach(gx=>grid.forEach(gz=>{
    if(gx===0&&gz===0) return;
    const x=gx*10+(gx%2===0?2:-2), z=gz*12+(gz%2===0?-2:2), y=CLOUD_ALT+(((gx+gz+5)%3)-1);
...
function LadderPrompts({enabled,setPrompt}: {enabled: boolean;setPrompt: (s: string | null) => void;}) {
  ...
  const baseW = 8, baseD = 8, ld = 0.5;
  return (
    <group>
      {houses.map(([x,z], i) => {
        const lx = x + baseW * 0.35, lz = z + baseD/2 + ld/2 + 0.02;
        return (
          <InteractAtPoint
            key={`ladder-${i}`}
        ...
        return (
          <InteractAtPoint
            ...
            onTrigger={() => {
              ...
              if (goingIn) {
                setInside(h.id);
                window.dispatchEvent(new CustomEvent("teleport-to", { detail: { x: h.insideSpawn.x, y: h.insideSpawn.y, z: h.insideSpawn.z }}));
              } else {
                setInside(null);
...
function DeskAndLamp({ x, z, darkMode }: { x: number; z: number; darkMode: boolean }) {
  ...
  const lampLightIntensity = darkMode ? 1.2 : 0.4;
  ...
  return (
    <group position={[x, 0, z]}>
      ...
      <mesh position={[BED.cx + BED.w/2 - 0.35, 0.62, BED.cz]}><boxGeometry args={[0.6, 0.12, 0.35]} /><meshBasicMaterial color="#ffffff" /></mesh>
...
function ThickSkySign({ text, rgbActive, darkMode }: { text: string; rgbActive: boolean; darkMode: boolean; }) {
  ...
  const spinning = useRef(false); const spinStart = useRef(0); const spinDuration = useRef(1200); const baseRotation = useRef(0);

  useEffect(() => {
    const startSpin = () => {
      if (!groupRef.current) return;
      spinning.current = true; spinStart.current = performance.now();
  ...
  if (!canvasRef.current) {
    ...
    canvasRef.current = canvas; ctxRef.current = canvas.getContext("2d"); texRef.current = new THREE.CanvasTexture(canvas);
    texRef.current.anisotropy = 8; texRef.current.needsUpdate = true;
  }
  ...
  const drawBanner = (phase: number) => {
    ...
    if (darkMode) { ctx.fillStyle = "#1d3b2a"; ctx.fillRect(0, 0, canvas.width, canvas.height * 0.25); ctx.fillStyle = "#245a38"; ctx.fillRect(0, 0, canvas.width, canvas.height * 0.18); }
    else { ctx.fillStyle = "#2e7d32"; ctx.fillRect(0, 0, canvas.width, canvas.height * 0.25); ctx.fillStyle = "#4caf50"; ctx.fillRect(0, 0, canvas.width, canvas.height * 0.18); }
    ctx.lineWidth = 40; ctx.strokeStyle = darkMode ? "#0b1220" : "#0f172a"; ctx.strokeRect(0, 0, canvas.width, canvas.height);
    if (rgbActive) {
      const seg = 32, perim = 2 * (canvas.width + canvas.height);
      for (let p = 0; p < perim; p += seg) {
        const hue = ((p / perim) * 360 + phase * 180) % 360;
        ctx.strokeStyle = `hsl(${hue},100%,60%)`; ctx.lineWidth = 60;
        ...
        while (s < e) {
          let x1=0,y1=0,x2=0,y2=0, left = e - s;
          if (s < canvas.width) { const d1 = Math.min(left, canvas.width - s); x1 = s; y1 = 0; x2 = s + d1; y2 = 0; drawEdge(x1,y1,x2,y2); s += d1; continue; }
          if (s < canvas.width + canvas.height) { const k = s - canvas.width; const d1 = Math.min(left, canvas.height - k); x1 = canvas.width; y1 = k; x2 = canvas.width; y2 = k + d1; drawEdge(x1,y1,x2,y2); s += d1; continue; }
          if (s < canvas.width * 2 + canvas.height) { const k = s - (canvas.width + canvas.height); const d1 = Math.min(left, canvas.width - k); x1 = canvas.width - k; y1 = canvas.height; x2 = canvas.width - (k + d1); y2 = canvas.height; drawEdge(x1,y1,x2,y2); s += d1; continue; }
          { const k = s - (canvas.width * 2 + canvas.height); const d1 = Math.min(left, canvas.height - k); x1 = 0; y1 = canvas.height - k; x2 = 0; y2 = canvas.height - (k + d1); drawEdge(x1,y1,x2,y2); s += d1; continue; }
        }
      }
    }
  ...
  useFrame(({ clock }) => {
    ...
    if (rgbActive) { phaseRef.current += 0.01; drawBanner(phaseRef.current); }
    if (spinning.current) {
      const now = performance.now(); const u = Math.min((now - spinStart.current) / spinDuration.current, 1);
      const eased = 1 - Math.pow(1 - u, 3);
      groupRef.current.rotation.y = baseRotation.current + eased * (Math.PI * 2);
      if (u >= 1) { spinning.current = false; groupRef.current.rotation.y = baseRotation.current + Math.PI * 2; }
    }
  });

  useEffect(() => { drawBanner(phaseRef.current); }, [text, rgbActive, darkMode]);
...
/* ---------- Grounded whiteboards ring (missing component) ---------- */
function GroundedWhiteboards({ setActiveBoard, darkMode, setPrompt }: { setActiveBoard: (id: string)=>void; darkMode:boolean; setPrompt:(s:string|null)=>void; }) {
  ...
  const plank = useMemo(()=>makePlankTexture(),[]);
  const labels = useMemo(()=>Object.fromEntries(defs.map(d => [d.id, makeBoardLabelTexture(d.label, darkMode)])), [darkMode]);
    ...
    <group>
      {defs.map((d,_i)=>(
        <group key={d.id} position={[d.x, BOARD_ALT, d.z]}>
          ...
          {/* interaction point slightly forward */}
          <InteractAtPoint
...
function makeBoardLabelTexture(text: string, dark: boolean){
  const w=1024, h=600, c=document.createElement("canvas"); c.width=w; c.height=h; const ctx=c.getContext("2d")!;
  // parchment
  const grad = ctx.createLinearGradient(0,0,0,h);
  grad.addColorStop(0, dark?"#0b1220":"#f2e4c7"); grad.addColorStop(1, dark?"#111827":"#e8d7b8");
  ctx.fillStyle = grad; ctx.fillRect(0,0,w,h);
  // border
  ctx.lineWidth = 24; ctx.strokeStyle = dark ? "#0f172a" : "#6b4f2f"; ctx.strokeRect(12,12,w-24,h-24);
  // title
  ctx.fillStyle = dark ? "#d1fae5" : "#111827";
  const title = wrapPixels(ctx, text.toUpperCase(), w-160, 100);
  ctx.font = "bold 80px 'Press Start 2P', monospace"; ctx.textAlign="center";
  ctx.fillText(title, w/2, h/2);
  const tex = new THREE.CanvasTexture(c); tex.anisotropy = 8; tex.needsUpdate = true; return tex;
}

function wrapPixels(ctx:CanvasRenderingContext2D, s:string, max:number, size:number){
  ctx.font = `bold ${size}px 'Press Start 2P', monospace`;
  if (ctx.measureText(s).width<=max) return s;
  // crude shrink until fits
  let f = size;
  while (f>30 && ctx.measureText(s).width>max){ f-=4; ctx.font = `bold ${f}px 'Press Start 2P', monospace`; }
  return s;
}
...
function makeFloorWoodTexture(){
  ...
  ctx.fillStyle = "#a07443"; ctx.fillRect(0,0,c.width,c.height);
  const rows = 14;
  for (let r=0;r<rows;r++){ const y=(r*c.height)/rows; ctx.fillStyle = r%2 ? "#8a6236" : "#b8834f"; ctx.fillRect(0,y,c.width,c.height/rows-2);
    ...
    for (let k=0;k<5;k++){ ctx.beginPath(); ctx.moveTo(0, y + (k + 1) * (c.height / rows) / 6); ctx.lineTo(c.width, y + (k + 1) * (c.height / rows) / 6); ctx.stroke(); }
  }
...
function makeVoxelGroundTexture(darkMode = false){
  const size=256, c=document.createElement("canvas"); c.width=size; c.height=size; const ctx=c.getContext("2d")!;
  const base = darkMode ? "#204d2c" : "#4caf50", speckMin = darkMode ? "rgba(5,30,15," : "rgba(20,100,40,";
...
function makeBrickTexture(){
  ...
  const rows=12, cols=16; ctx.fillStyle="#8c3f2c";
